...
AuthorizeHandler.prototype.getClient = function(request) {
  var clientId = request.body.app_id || request.query.app_id || request.body.client_id || request.query.client_id;
  ...
  return promisify(this.model.getClient, 2).call(this.model, clientId, null)
    .then(function(client) {
      ...
      if (redirectUri) {
        ...
        const needMatchKey = ['redirectUri', 'host', 'pathname'];

        const matched = client.redirectUris.some(function(item) {
          const parsed = url.parse(item);
          return needMatchKey.every(function(key) {
            return uri[key] === parsed[key];
          })
        });
...
AuthorizeHandler.prototype.updateResponse = function(response, redirectUri, responseType, state) {
  if (responseType && state) {
    ...
    redirectUri.query = typeof query === 'object' && query !== null ? query : querystring.parse(query);
