...
TokenHandler.prototype.getClient = function(request, response) {
  ...
  if (!credentials.clientId) {
    throw new InvalidRequestError('Missing parameter: `app_id` or `client_id`');
  }

  if (this.isClientAuthenticationRequired(grantType) && !credentials.clientSecret) {
    throw new InvalidRequestError('Missing parameter: `app_secret` or `client_secret`');
  }

  if (!is.vschar(credentials.clientId)) {
    throw new InvalidRequestError('Invalid parameter: `app_id` or `client_id`');
  }

  if (credentials.clientSecret && !is.vschar(credentials.clientSecret)) {
    throw new InvalidRequestError('Invalid parameter: `app_secret` or `client_secret`');
  }
 ...
 * the `app_id` and `app_secret` can be embedded in the body.
 *
...
TokenHandler.prototype.getClientCredentials = function(request) {
  ...
  if (request.body.app_id && request.body.app_secret || request.body.client_id && request.body.client_secret) {
    return { 
      clientId: request.body.app_id || request.body.client_id, 
      clientSecret: request.body.app_secret || request.body.client_secret 
    };
  }

  if (!this.isClientAuthenticationRequired(grantType)) {
    if(request.body.app_id || request.body.client_id) {
      return { clientId: request.body.app_id || request.body.client_id };
    }
  }
