...
TokenHandler.prototype.getClient = function(request, response) {
  ...
  if (!credentials.clientId) {
    throw new InvalidRequestError('Missing parameter: `app_id`');
  }

  if (this.isClientAuthenticationRequired(grantType) && !credentials.clientSecret) {
    throw new InvalidRequestError('Missing parameter: `app_secret`');
  }

  if (!is.vschar(credentials.clientId)) {
    throw new InvalidRequestError('Invalid parameter: `app_id`');
  }

  if (credentials.clientSecret && !is.vschar(credentials.clientSecret)) {
    throw new InvalidRequestError('Invalid parameter: `app_secret`');
  }
...
TokenHandler.prototype.getClientCredentials = function(request) {
  ...
  if (request.body.app_id && request.body.app_secret) {
    return { clientId: request.body.app_id, clientSecret: request.body.app_secret };
  }

  if (!this.isClientAuthenticationRequired(grantType)) {
    if(request.body.app_id) {
      return { clientId: request.body.app_id };
    }
  }
